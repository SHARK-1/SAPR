...
namespace Kompas
{
    ...
    public class Builder
    {
        ...
        private void CreateGuide()
        {
            ...
            double[,] points = MathPoints(offset);

            double radius = 2;

            ksCON[] con = MathConPoints(points, radius);
            int con14Index = _guideParameters.GuideAngle < 180 ? 3 : 2;
            short con14rotation = _guideParameters.GuideAngle < 180
                ? (short)1
                : (short)-1;
            ...
            if (_guideParameters.GuideAngle != 180)
            {
                ...
                _document2D.ksLineSeg(con[0].GetX2(1),
                    con[0].GetY2(1) + offset,
                    con[1].GetX1(con14Index),
                    con[1].GetY1(con14Index) + offset, 1);
            ...
            //Выдавливание
            var entityExtrude = (ksEntity)_part.
        ...
        private void CutAttachmentStroke()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (i < 3)
                {
                    len = 0.5 * (_guideParameters.GuideWidth
                        + _guideParameters.AttachmentStrokeWidth);
                }
                else
                {
                    len = 0.5 * (_guideParameters.GuideWidth +
            ...
            for (int i = 0; i < 2; i++)
            {
                _document2D.ksLineSeg(attachmentpoints[1+i, 0],
               attachmentpoints[1+i, 1] + offset,
               attachmentpoints[4+i, 0],
               attachmentpoints[4+i, 1] + offset, 1);
            }
            for (int i = 0; i < 4; i += 3)
            {
                short dir = i == 0 ? (short)1 : (short)-1;
                _document2D.ksArcByPoint(attachmentpoints[i, 0],
                                attachmentpoints[i, 1] + offset,
                                halfStrokeWidth,
                                attachmentpoints[1 + i, 0],
                                attachmentpoints[1 + i, 1] + offset,
                                attachmentpoints[2 + i, 0],
                                attachmentpoints[2 + i, 1] + offset,
                                dir, 1);
            }
        ...
        /// <summary>
        /// Высчитывание координат для Con
        /// </summary>
        /// <param name="points">Точки прямых</param>
        /// <param name="radius">Радиус сгругления</param>
        /// <returns>Массив Con</returns>
        private ksCON[] MathConPoints(double[,] points, double radius)
        {
            ksCON[] con = new ksCON[5]
            {
                _kompasConnector.Kompas.GetParamStruct(CON_STRUCT_TYPE),
                _kompasConnector.Kompas.GetParamStruct(CON_STRUCT_TYPE),
                _kompasConnector.Kompas.GetParamStruct(CON_STRUCT_TYPE),
                _kompasConnector.Kompas.GetParamStruct(CON_STRUCT_TYPE),
                _kompasConnector.Kompas.GetParamStruct(CON_STRUCT_TYPE)
            };
            ksMathematic2D ksmathematic =
                _kompasConnector.Kompas.GetMathematic2D();
        ...
        /// <summary>
        /// Высчитывание крайних точек
        /// </summary>
        /// <param name="offset">Половина ширины направляющей</param>
        /// <returns>Двумерный массив точек</returns>
        private double[,] MathPoints(double offset)
        {
            ...
            var line =
                _document2D.ksLineSeg(0, 0, points[2, 0], points[2, 1], 6);
            ksMathematic2D ksmathematic =
                _kompasConnector.Kompas.GetMathematic2D();
        ...
        private void CreateHole()
        {
            ...
            double[,] points = new double[5, 2] {
                ...
                { -_guideParameters.GuideLength+
                2*wallReserve+headReserve, 0 },
                { -_guideParameters.GuideLength+
                2*wallReserve+headReserve,wallReserve+headReserve/2 },
                { -_guideParameters.GuideLength
                +wallReserve+headReserve/2,wallReserve+headReserve/2 } };

            _document2D.ksLineSeg(
                points[0, 0], points[0, 1], points[1, 0], points[1, 1], 1);
            _document2D.ksLineSeg(
                points[0, 0], points[0, 1], points[2, 0], points[2, 1], 1);
            _document2D.ksLineSeg(
                points[2, 0], points[2, 1], points[3, 0], points[3, 1], 1);
            _document2D.ksArcByPoint(
                points[4, 0], points[4, 1],
                _guideParameters.HoleDiameter * 0.75 + 5,
                points[1, 0], points[1, 1], points[3, 0], points[3, 1], -1, 1);
            _document2D.ksCircle(
                points[4, 0], points[4, 1],
                _guideParameters.HoleDiameter / 2, 1);
